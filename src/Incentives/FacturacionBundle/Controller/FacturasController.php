...
class FacturasController extends Controller
{
    ...
    public function generarSegmentadoAction(Request $request)
    {
        ...
        //Cotizaciones pendientes por facturacion
        $qb = $em->createQueryBuilder(); 
        ...
        $qb->groupBy('p.id');
        $str_filtro = "cp.facturaProducto IS NULL AND cp.estado in (2,6,5) AND s.estado in (2,3,5)";
        $qb->where($str_filtro);
        $Cotizaciones = $qb->getQuery()->getResult(\Doctrine\ORM\Query::HYDRATE_ARRAY);
        
        //echo "<pre>"; print_r($Cotizaciones); echo "</pre>"; 
        //exit;
        
        $Solicitudes = $Cotizaciones;
        
        //$Solicitudes = array_intersect($Redenciones,$Requisiciones); 
        ...
        return $this->render('IncentivesFacturacionBundle:Facturas:generarsegmentado.html.twig', 
            array('redenciones' => $Redenciones, 'solicitudes' => $Solicitudes, 'logisticas' => $Logistica));
    }
   ...
   public function detalleSolicitudesAction($id)
    ...
    public function detalleFacturaSolicitudesAction($programa)
    {
        ...
        $idPrograma =($this->get('request')->request->get('programa'));
                
        //Cotizaciones
        //CONTAR CANTIDADES POR PRODUCTO Y PRECIOS
        $qb = $em->createQueryBuilder(); 
        $qb->select('cp','c','s','p','pd');
        $qb->from('IncentivesSolicitudesBundle:CotizacionProducto','cp');
        $qb->leftJoin('cp.cotizacion', 'c');
        $qb->leftJoin('c.solicitud', 's');
        $qb->leftJoin('cp.producto', 'pd');
        $qb->leftJoin('s.programa', 'p');
        $str_filtro = "cp.facturaProducto IS NULL AND cp.estado in (2,6,5) AND s.estado IN (2,3,5) AND s.programa=".$programa->getId();
        $qb->where($str_filtro);
        $ProductosCotizaciones = $qb->getQuery()->getResult(\Doctrine\ORM\Query::HYDRATE_ARRAY);
        
        //echo "<pre>"; print_r($ProductosCotizaciones); echo "</pre>";
        //exit;
        
        //Requisicones
        //CONTAR CANTIDADES POR PRODUCTO Y PRECIOS
        $qb = $em->createQueryBuilder(); 
        $qb->select('rp','r','s','p','pd');
        $qb->from('IncentivesSolicitudesBundle:RequisicionProducto','rp');
        $qb->leftJoin('rp.requisicion', 'r');
        $qb->leftJoin('r.solicitud', 's');
        $qb->leftJoin('rp.producto', 'pd');
        $qb->leftJoin('s.programa', 'p');
        $str_filtro = "rp.facturaProducto IS NULL AND s.programa=".$programa->getId();
        $qb->where($str_filtro);
        $ProductosRequisiciones = $qb->getQuery()->getResult(\Doctrine\ORM\Query::HYDRATE_ARRAY);
        
        //echo "<pre>"; print_r($ProductosRequisiciones); echo "</pre>"; 
        //exit;
        ...
        $qb->from('IncentivesSolicitudesBundle:Cotizacion','c');
        $qb->leftJoin('c.solicitud', 's');
        $str_filtro = "c.facturaLogistica IS NULL AND c.logistica IS NOT NULL AND c.logistica!=0 AND s.estado IN (2,3,5) AND s.programa=".$programa->getId();
        $qb->where($str_filtro);
        $Logistica = $qb->getQuery()->getResult(\Doctrine\ORM\Query::HYDRATE_ARRAY);
      ...
      return $this->render('IncentivesFacturacionBundle:Facturas:detallefacturasolicitudes.html.twig', 
          array('requisiciones' => $ProductosRequisiciones,'cotizaciones' => $ProductosCotizaciones, 'logistica' => $Logistica
      ));
